...
const investmentSlides = [
  { sector: "Tourism", title: "EXPLORE: TOURISM INVESTMENT OPPORTUNITIES IN BOTSWANA", description: "Discover premium eco-tourism, hospitality and conservation-led opportunities across Botswana’s remarkable destinations.", image: "/thabo-opportunities/tourism-01.jpeg", script: "Welcome to investment opportunities in Botswana. Explore tourism opportunities where conservation, culture and premium hospitality create lasting value. BITC assists investors through one-stop services for investing in Botswana. Click Chat to talk to Thabo and specify your preferred language for communication." },
  { sector: "Mining", title: "EXPLORE: MINING INVESTMENT OPPORTUNITIES IN BOTSWANA", description: "Explore responsible mineral development, value addition, infrastructure and partnership opportunities in a stable market.", image: "/thabo-opportunities/mining-01.jpeg", script: "Welcome to investment opportunities in Botswana. Explore mining opportunities supported by a stable business environment and strong partnership potential. BITC assists investors through one-stop services for investing in Botswana. Click Chat to talk to Thabo and specify your preferred language for communication." },
  { sector: "Agriculture", title: "EXPLORE: AGRICULTURE INVESTMENT OPPORTUNITIES IN BOTSWANA", description: "Invest across production, livestock, agri-processing, food security and sustainable regional value chains.", image: "/thabo-opportunities/agriculture-01.jpeg", script: "Welcome to investment opportunities in Botswana. Explore agriculture opportunities from livestock and production to agri-processing and sustainable value chains. BITC assists investors through one-stop services for investing in Botswana. Click Chat to talk to Thabo and specify your preferred language for communication." },
  { sector: "Tourism", title: "EXPLORE: TOURISM INVESTMENT OPPORTUNITIES IN BOTSWANA", description: "Connect with world-class wilderness experiences and the services, lodges and transport networks that support them.", image: "/thabo-opportunities/tourism-02.jpeg", script: "Welcome to investment opportunities in Botswana. Discover tourism opportunities built around world-class wilderness, hospitality and unforgettable visitor experiences. BITC assists investors through one-stop services for investing in Botswana. Click Chat to talk to Thabo and specify your preferred language for communication." },
  { sector: "Mining", title: "EXPLORE: MINING INVESTMENT OPPORTUNITIES IN BOTSWANA", description: "Find opportunities spanning exploration, technology, services, beneficiation and responsible mine operations.", image: "/thabo-opportunities/mining-02.jpeg", script: "Welcome to investment opportunities in Botswana. Discover mining opportunities including exploration, services, technology and local value addition. BITC assists investors through one-stop services for investing in Botswana. Click Chat to talk to Thabo and specify your preferred language for communication." },
  { sector: "Agriculture", title: "EXPLORE: AGRICULTURE INVESTMENT OPPORTUNITIES IN BOTSWANA", description: "Grow with Botswana through horticulture, modern farming, logistics, processing and export-ready agribusiness.", image: "/thabo-opportunities/agriculture-02.jpeg", script: "Welcome to investment opportunities in Botswana. Grow through modern agriculture, horticulture, logistics, processing and export-ready agribusiness. BITC assists investors through one-stop services for investing in Botswana. Click Chat to talk to Thabo and specify your preferred language for communication." },
] as const;
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Allow the complete BITC introduction and sector message to finish before
    // the next opportunity replaces it.
    const timer = window.setInterval(() => setActiveOpportunityIndex(current => (current + 1) % investmentSlides.length), 28_000);
    ...
    <div className={`min-h-screen overflow-hidden transition-colors duration-300 ${isDark ? "bg-[#05070d] text-slate-100" : "light-theme bg-[#eef4f7] text-slate-900"}`}>
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl flex-col px-5 py-6 sm:px-8 sm:py-8">
        <section aria-labelledby="top-invest-banner-title" className="relative -mx-5 mb-5 min-h-[172px] w-[calc(100%+2.5rem)] overflow-hidden rounded-none border border-cyan-200/15 bg-[#061118] px-5 py-7 shadow-[0_22px_60px_rgba(2,8,23,0.28)] sm:-mx-8 sm:min-h-[198px] sm:w-[calc(100%+4rem)] sm:rounded-[24px] sm:px-8 sm:py-8">
          <div className="pointer-events-none absolute inset-0 bg-cover bg-center opacity-80" style={{ backgroundImage: "url('/thabo-opportunities/bitc-headquarters.jpeg')" }} />
        ...
        <section aria-labelledby="investment-stream-title" className={`investment-stream mx-auto mb-8 w-full max-w-6xl overflow-hidden rounded-[34px] border shadow-[0_30px_90px_rgba(2,8,23,0.4)] ${isDark ? "border-amber-200/20 bg-[#10160f]/90" : "border-amber-950/10 bg-[#fffaf0]/90 shadow-amber-950/10"}`}>
          <div className="flex items-center justify-between gap-4 border-b px-5 py-4 sm:px-7">
            <div><p className={`text-[10px] font-semibold uppercase tracking-[0.28em] ${isDark ? "text-amber-200/80" : "text-amber-900/75"}`}>THABO · Investment channel</p><h2 id="investment-stream-title" className={`mt-1 text-xl font-semibold tracking-tight sm:text-2xl ${isDark ? "text-white" : "text-slate-900"}`}>Botswana opportunities, live and continuously rotating</h2></div>
          ...
          <div className="relative min-h-[520px] overflow-hidden sm:min-h-[600px]">
            {investmentSlides.map((slide, index) => <img key={slide.image} src={slide.image} alt={index === activeOpportunityIndex ? `${slide.sector} investment opportunity in Botswana` : ""} aria-hidden={index !== activeOpportunityIndex} className={`absolute inset-0 h-full w-full object-cover transition-all duration-1000 ${index === activeOpportunityIndex ? "scale-100 opacity-100" : "scale-105 opacity-0"}`} />)}
            <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(4,10,8,0.94)_0%,rgba(4,10,8,0.78)_42%,rgba(4,10,8,0.24)_76%,rgba(4,10,8,0.42)_100%)]" />
            <div className="absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-[#07100c] to-transparent" />
            <div className="relative z-10 flex min-h-[520px] max-w-3xl flex-col justify-end p-6 pb-8 sm:min-h-[600px] sm:p-12">
              <div className="mb-5 flex items-center gap-3"><span className="rounded-full border border-amber-200/30 bg-amber-200/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.24em] text-amber-100">{activeOpportunity.sector}</span><span className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/55">Slide {activeOpportunityIndex + 1} of {investmentSlides.length}</span></div>
              <p className="text-xs font-bold uppercase tracking-[0.24em] text-amber-200/80">THABO voice-over</p>
              <h3 className="mt-3 max-w-3xl text-3xl font-black leading-[1.05] tracking-tight text-white sm:text-5xl">{activeOpportunity.title}</h3>
              <p className="mt-5 max-w-2xl text-sm leading-7 text-slate-200/85 sm:text-base">{activeOpportunity.description}</p>
              <p className="mt-3 max-w-2xl text-xs font-semibold uppercase tracking-[0.12em] text-amber-100/80">BITC provides one-stop investor services · Click Chat and tell Thabo your preferred communication language</p>
              <div className="mt-7 flex flex-wrap items-center gap-3"><button type="button" onClick={() => { const next = !voiceoverEnabled; setVoiceoverEnabled(next); if (!next) stopInvestmentVoiceover(); }} className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-black/25 px-4 py-3 text-xs font-bold text-white backdrop-blur-md transition hover:bg-white/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-200" aria-pressed={voiceoverEnabled}>{voiceoverEnabled ? <Volume2 className="size-4" /> : <VolumeX className="size-4" />}{voiceoverEnabled ? "Voice-over on" : "Enable voice-over"}</button><button type="button" onClick={openChat} className="inline-flex items-center gap-2 rounded-full bg-amber-300 px-5 py-3 text-xs font-black text-slate-950 transition hover:bg-amber-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-100"><MessageCircle className="size-4" /> Start chat with Thabo</button></div>
              <div className="mt-8 flex gap-2" aria-label="Choose investment slide">{investmentSlides.map((slide, index) => <button key={`${slide.sector}-${index}`} type="button" onClick={() => setActiveOpportunityIndex(index)} aria-label={`Show ${slide.sector} opportunity ${index + 1}`} className={`h-1.5 rounded-full transition-all ${index === activeOpportunityIndex ? "w-12 bg-amber-300" : "w-6 bg-white/30 hover:bg-white/55"}`} />)}</div>
            </div>
            <div className="investment-stream-progress" aria-hidden="true"><span key={activeOpportunityIndex} /></div>
          </div>
        </section>
